...
## 2026-03-22 - Explicitly Marking Required Fields in Streamlit
**Learning:** Streamlit lacks native required field visual indicators like standard web forms. Users can be confused about which fields are necessary before submitting a form or triggering an action.
**Action:** Always explicitly mark required fields by adding an asterisk (*) to the label (e.g., `label="Order Number *"`) and providing a `help="Required field"` parameter to explicitly state the requirement, improving both UX and accessibility.
