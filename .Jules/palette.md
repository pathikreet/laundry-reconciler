...
## 2026-03-24 - Streamlit Form Notifications
**Learning:** Using `st.success()` for sequential entry forms (like adding items to a list) causes poor UX because the message only flashes briefly before the required `st.rerun()` clears it. This leaves the user unsure if their action succeeded.
**Action:** Use `st.toast()` instead of `st.success()` for confirming repetitive add/update actions. Toasts survive the immediate script rerun and provide clear, unobtrusive feedback.

## 2026-03-24 - Accessibility of Required Fields in Streamlit
**Learning:** Streamlit inputs do not natively mark fields as required or add asterisks. This hurts accessibility and leads to validation errors.
**Action:** Explicitly add `*` to the label strings of required fields and use the `help="Required field"` parameter on the input widget to provide a tooltip for extra clarity.
