...
## 2026-03-22 - Form Clearing and Toast Notifications in Streamlit
**Learning:** Sequential manual entry forms (like adding multiple notepad records) create friction if inputs aren't automatically cleared after submission. Additionally, using `st.success` followed immediately by `st.rerun` causes the success message to flash and disappear before the user can read it. Wrapping inputs in `st.form(clear_on_submit=True)` automatically handles resetting the form state. Replacing `st.success` with `st.toast` provides a non-blocking, persistent notification that survives the necessary `st.rerun()` needed to update the displayed list of entries.
**Action:** Use `st.form(clear_on_submit=True)` combined with `st.toast()` for any repetitive data entry component where state needs to be cleared and success feedback needs to be visible without interrupting the flow.
