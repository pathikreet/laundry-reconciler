...
## 2026-04-24 - Form Field Required Indicators
**Learning:** Streamlit forms and input widgets do not automatically display visual required indicators (like asterisks) or helpful tooltips indicating a field is mandatory, even when validation logic exists downstream. This creates an accessibility barrier and poor UX as users don't know what is required until they encounter an error.
**Action:** Always append an asterisk `*` to the label string of implicitly required inputs (e.g. `label="Order Number *"`), and attach the explicit `help="Required field"` tooltip argument to guide users proactively.
