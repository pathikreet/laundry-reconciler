...
## 2026-04-16 - Explicitly Mark Required Fields\n**Learning:** Streamlit's `st.text_input` and other input widgets do not natively indicate whether they are required visually, even if the underlying logic enforces validation. This can cause frustration when users submit forms only to encounter error messages for missed fields.\n**Action:** Add an asterisk (`*`) to the label of required fields and use the `help` parameter (e.g., `help="Required field"`) to provide a tooltip, improving both clarity and accessibility.
