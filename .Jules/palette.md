...
## 2026-04-30 - Explicitly Marking Required Fields in Streamlit
**Learning:** Streamlit forms and input widgets do not natively provide visual indicators for required fields, leaving users guessing which inputs are mandatory. Furthermore, accessibility best practices dictate clear, semantic indicators.
**Action:** Always append an asterisk (`*`) to the label of required Streamlit text inputs (e.g., `"Order Number *"`) and include the `help="Required field"` parameter to provide explicit visual and tooltip guidance.
