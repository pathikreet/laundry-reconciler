...
\n## 2026-05-12 - Explicit Required Fields and Toast Notifications\n**Learning:** In Streamlit, `st.text_input` does not natively indicate if a field is required, leading to accessibility and usability issues. Additionally, using `st.success` immediately before an `st.rerun` causes the message to flash and disappear, giving poor feedback.\n**Action:** Manually mark required fields by adding an asterisk to the label and using the `help="Required field"` kwarg. Replace transient `st.success` messages with `st.toast` for smoother, persistent notifications across reruns.
