...
## 2026-05-01 - Explicitly Marking Required Fields
**Learning:** Streamlit forms lack native visual indicators for required fields, which causes frustration when users submit incomplete forms. Adding asterisks to labels isn't enough for accessibility; combining an asterisk in the label with a `help` parameter explicitly stating 'Required field' ensures both sighted and screen reader users understand the requirement before submission.
**Action:** In Streamlit apps, explicitly mark required input fields using an asterisk (*) in the label and the `help="Required field"` parameter to improve accessibility and UX.
