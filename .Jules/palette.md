...
## 2026-05-10 - Toast over Success before Rerun
**Learning:** Using `st.success` immediately before an `st.rerun()` creates a jarring UX where the success message flashes and vanishes before the user can read it.
**Action:** Use `st.toast` for notifications that immediately precede a script rerun, as toasts persist across reruns.
