...
def render_notepad_step(session_db, is_unlocked):
    ...
    with tab_manual:
        ...
        with st.expander("➕ Add Order Details", expanded=len(st.session_state.notepad_entries) == 0):
            with st.form("notepad_entry_form", clear_on_submit=True):
                ...
                if st.form_submit_button("➕ Add Order Details", type="primary"):
                    if not entry_order:
                        st.toast("Please enter an Order Number", icon="⚠️")
                    else:
